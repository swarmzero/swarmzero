...
async def process_query(body: Body):
    ...
    redis_client.hset(TASK_STATUS, task_id, json.dumps(task_status))
    for subtask in subtasks:
        task_data = json.dumps(
            {"task_id": task_id, "subtask": subtask["task"], "agent_name": subtask["agent"]}
        )
        redis_client.rpush(TASK_QUEUE, task_data)
