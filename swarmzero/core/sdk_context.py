...
from swarmzero.core.services.database.database import engine
...
class SDKContext:
    ...
    async def load_default_utility(self):
        ...
        if self.get_utility("db_manager") is None:
            ...
            async with get_db() as db:
                db_manager = await setup_chats_table(db)
                ...
                self.utilities["db_manager"]["cleanup"] = cleanup
    ...
    async def cleanup(self):
        ...
        # Dispose engine only during final cleanup
        await engine.dispose()
